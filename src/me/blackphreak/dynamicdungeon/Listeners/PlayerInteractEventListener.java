		...
		if (e.getPlayer().getLocation().getWorld().getName()
			...
			if (dg != null) {
				...
				dg.getTriggers().stream()
						.filter(t -> t instanceof InteractTrigger)
						.filter(DungeonTrigger::isActivated)
						.filter(DungeonTrigger::isActionMade)
						.filter(t -> t.condition(dg, e))
						.forEach(t -> t.action(dg, new ActionNeeded(e.getPlayer(), t.getLocation()))
						);
					...
					case "[--dungeon--]":
						if (sign.getLine(1).startsWith("DGID: ")) {
							...
							if (sessionID == -1) {
								...
								gb.dungeonCreating.add(sessionID);
								gb.dungeonPlaying.put(e.getPlayer(), sessionID);
							} else {
